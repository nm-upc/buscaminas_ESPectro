...
void drawMenu(int bestScore) {
    ...
    tft.setTextSize(4);
    const char* linia1 = "BUSCA";
    const char* linia2 = "MINES";
...
#define MINES_ROWS 13
#define MINES_TOTAL 15
#define CELL_W 28
#define CELL_H 28
#define GRID_OX ((SCREEN_W - MINES_COLS*CELL_W)/2)
#define GRID_OY 70

uint8_t mnBoard[MINES_ROWS][MINES_COLS]; // bit0=mina, bit1=descobert, bit2=bandera
int mnCurX=0, mnCurY=0;
int mnScore=0;
bool mnInitialized=false;

void mnInit(int safeR, int safeC) {
    memset(mnBoard,0,sizeof(mnBoard));
    int placed=0;
    while (placed<MINES_TOTAL) {
        int r=random(MINES_ROWS), c=random(MINES_COLS);
        if (!(mnBoard[r][c]&1) && !(abs(r-safeR)<=1 && abs(c-safeC)<=1)) {
            mnBoard[r][c]|=1; placed++;
        }
    }
}
int mnAdj(int r, int c) {
    int cnt=0;
    for (int dr=-1;dr<=1;dr++) for (int dc=-1;dc<=1;dc++) {
        int nr=r+dr,nc=c+dc;
        if (nr>=0&&nr<MINES_ROWS&&nc>=0&&nc<MINES_COLS&&(mnBoard[nr][nc]&1)) cnt++;
    }
    return cnt;
}
void mnReveal(int r, int c);
void mnReveal(int r, int c) {
    if (r<0||r>=MINES_ROWS||c<0||c>=MINES_COLS) return;
    if (mnBoard[r][c]&2) return;
    if (mnBoard[r][c]&4) return;
    mnBoard[r][c]|=2;
    mnScore++;
    if (mnAdj(r,c)==0 && !(mnBoard[r][c]&1))
        for (int dr=-1;dr<=1;dr++) for (int dc=-1;dc<=1;dc++) mnReveal(r+dr,c+dc);
}
static const uint16_t ADJ_COLORS[9] = {
    TFT_BLACK,
    TFT_BLUE, TFT_GREEN, TFT_RED,
    0x000F /*dark blue*/, 0xA000/*dark red*/, TFT_CYAN, TFT_MAGENTA, TFT_DARKGREY
};
void mnDrawGrid(bool showAll) {
    tft.startWrite();
    for (int r=0;r<MINES_ROWS;r++) for (int c=0;c<MINES_COLS;c++) {
        int x=GRID_OX+c*CELL_W, y=GRID_OY+r*CELL_H;
        uint8_t cell=mnBoard[r][c];
        bool disc=(cell&2)||showAll;
        bool mina=(cell&1);
        bool flag=(cell&4);
        if (!disc) {
            tft.fillRect(x+1,y+1,CELL_W-2,CELL_H-2, tft.color565(80,80,80));
            if (flag) { tft.setTextColor(TFT_RED,tft.color565(80,80,80)); tft.setTextSize(2); tft.setCursor(x+5,y+5); tft.print("F"); }
        } else {
            if (mina) { tft.fillRect(x+1,y+1,CELL_W-2,CELL_H-2,TFT_RED); tft.setTextColor(TFT_BLACK,TFT_RED); tft.setTextSize(2); tft.setCursor(x+5,y+5); tft.print("*"); }
            else {
                tft.fillRect(x+1,y+1,CELL_W-2,CELL_H-2,tft.color565(30,30,30));
                int adj=mnAdj(r,c);
                if (adj>0) {
                    tft.setTextColor(ADJ_COLORS[adj], tft.color565(30,30,30));
                    tft.setTextSize(2); tft.setCursor(x+7,y+5); tft.printf("%d",adj);
                }
            }
        }
        tft.drawRect(x,y,CELL_W,CELL_H, tft.color565(50,50,50));
    }
    // Cursor
    int cx=GRID_OX+mnCurX*CELL_W, cy=GRID_OY+mnCurY*CELL_H;
    tft.drawRect(cx,cy,CELL_W,CELL_H,TFT_YELLOW);
    tft.drawRect(cx+1,cy+1,CELL_W-2,CELL_H-2,TFT_YELLOW);
    tft.endWrite();
}
...
void runGame() {
    ...
    mnScore=0; mnInitialized=false; mnCurX=5; mnCurY=6;
    memset(mnBoard,0,sizeof(mnBoard));

    tft.fillScreen(TFT_BLACK);
    tft.setTextColor(tft.color565(255,80,0), TFT_BLACK); tft.setTextSize(2);
    tft.setCursor(10,5); tft.printf("BUSCAMINAS   Rec:%d", bestScore);
    tft.setTextSize(1); tft.setTextColor(TFT_WHITE,TFT_BLACK);
    tft.setCursor(10,30); tft.print("A=descobrir  Joy SW=bandera  B=sortir");
    mnDrawGrid(false);

    unsigned long lastMove=millis();
    bool joyPrev=false, aPrev=false;
    ...
    while (true) {
        ...
        // Joystick button = flag
        if (joyBtn && !joyPrev) {
            if (!(mnBoard[mnCurY][mnCurX]&2)) {
                mnBoard[mnCurY][mnCurX] ^= 4;
                mnDrawGrid(false);
            }
        }
        joyPrev=joyBtn;
        ...
        // BTN A = uncover
        if (btnA && !aPrev) {
            if (!mnInitialized) { mnInit(mnCurY,mnCurX); mnInitialized=true; }
            if (!(mnBoard[mnCurY][mnCurX]&4)) {
                if (mnBoard[mnCurY][mnCurX]&1) {
                    // BOOM
                    mnReveal(mnCurY,mnCurX);
                    mnDrawGrid(true);
                    ...
                    mnReveal(mnCurY,mnCurX);
                    // Check win: totes les no-mina descobertes
                    int total_safe=MINES_ROWS*MINES_COLS-MINES_TOTAL;
                    bool won=true;
                    for (int r=0;r<MINES_ROWS;r++) for (int c=0;c<MINES_COLS;c++)
                        if (!(mnBoard[r][c]&1) && !(mnBoard[r][c]&2)) won=false;
                    mnDrawGrid(false);
                    tft.setTextSize(1); tft.setTextColor(TFT_WHITE,TFT_BLACK);
                    tft.setCursor(10,50); tft.printf("Descobertes: %d/%d", mnScore, total_safe);
                    if (won) {
